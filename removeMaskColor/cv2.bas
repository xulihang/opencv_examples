...
public Sub countNonZero(src As cvMat) As Int
	Return Core.RunMethod("countNonZero", Array(src.getJO))
End Sub

public Sub calcHist(src As cvMat,dst As cvMat)
	...
	Imgproc.RunMethod("calcHist", Array(srcList, channels, emptyMask, dst.getJO, histSize, ranges))
End Sub

Sub findNonZero(src As cvMat,target As cvMat)
	Core.RunMethod("findNonZero",Array(src.JO,target.jo))
End Sub
...
Public Sub matOnes(params() As Object) As cvMat
	Return matJO2mat(matStatic.RunMethodJO("ones",params))
End Sub

Public Sub multiply(src1 As cvMat, src2 As JavaObject, dst As cvMat)
	Core.RunMethod("multiply",Array(src1.JO,src2,dst.JO))
End Sub
...
public Sub adaptiveThreshold(src As cvMat,dst As cvMat,blockSize As Int,C As Double)
	Imgproc.RunMethod("adaptiveThreshold",Array(src.JO,dst.JO,255.0,procEnum("ADAPTIVE_THRESH_GAUSSIAN_C"),procEnum("THRESH_BINARY"),blockSize,C))
End Sub
...
Public Sub seamlessClone(source As cvMat,destination As cvMat, result As cvMat,mask As cvMat, center As JavaObject)
	'seamlessClone​(Mat src, Mat dst, Mat mask, Point p, Mat blend, int flags)
	Photo.RunMethod("seamlessClone",Array(source.JO,destination.JO,mask.jo,center,result.JO,Photo.GetField("MIXED_CLONE")))
End Sub
...
Public Sub ScalarSingle(v As Double) As JavaObject
	Dim sca As JavaObject
	sca.InitializeNewInstance("org.opencv.core.Scalar",Array(v))
	Return sca
End Sub
...
public Sub getRotationMatrix2D(center As JavaObject,angle As Double,scale As Double) As cvMat
	Dim jo As JavaObject = Imgproc.RunMethod("getRotationMatrix2D",Array(center, angle, scale))
	Return matJO2mat(jo)
End Sub

public Sub warpAffine(src As cvMat,dst As cvMat, rotationMatrix As cvMat, newSize As JavaObject)
	Imgproc.RunMethod("warpAffine",Array(src.JO, dst.JO, rotationMatrix.JO, newSize))
End Sub

public Sub warpAffineWithBorder(src As cvMat,dst As cvMat, rotationMatrix As cvMat, newSize As JavaObject,borderColor As JavaObject)
	Dim flags As Int = procEnum("INTER_LINEAR")
	Dim boderMode As Int =  coreEnum("BORDER_CONSTANT")
	Imgproc.RunMethod("warpAffine",Array(src.JO, dst.JO, rotationMatrix.JO, newSize,flags,boderMode,borderColor))
End Sub

Public Sub compare(src1 As cvMat, src2 As JavaObject, dst As cvMat, cmpop As Int)
	Core.RunMethod("compare", Array(src1.JO, src2, dst.JO, cmpop))
End Sub

Public Sub multiplyScalar(src As cvMat, ScalarValue As Double, dst As cvMat)
	Core.RunMethod("multiply", Array(src.JO, ScalarSingle(ScalarValue), dst.JO))
End Sub

Public Sub convertTo(src As cvMat, dst As cvMat, rtype As Int, alpha As Double, beta As Double)
	src.JO.RunMethod("convertTo", Array(dst.JO, rtype, alpha, beta))
End Sub

Public Sub normalize(src As cvMat, dst As cvMat, alpha As Double, beta As Double, norm_type As Int, dtype As Int, mask As JavaObject)
	Core.RunMethod("normalize", Array(src.JO, dst.JO, alpha, beta, norm_type, dtype, mask))
End Sub

Public Sub getDepth(mat As cvMat) As Int
	Return mat.JO.RunMethod("depth", Null)
End Sub
